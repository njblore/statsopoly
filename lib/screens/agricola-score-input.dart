...
class _AgricolaScoreInputState extends State<AgricolaScoreInput> {
                              ...
                              Flexible(
                                child: TextFormField(
                                  ...
                                  onSaved: (String value) {
                                    setState(() {
                                      formData.datePlayed = formatDate(value);
                                    });
                                  },
                                ),
                              ),
  ...
  Future<http.Response> postGameScore(GameScore jsonScores) async {
    setState(() {
      loading = true;
    });

    var fetchedScores = await _fetchGames();
    var jsonScorez = jsonDecode(fetchedScores.body);

    List<GameScore> fetchedGameList = AllGames.fromJson(jsonScorez).gameScores;

    fetchedGameList.add(jsonScores);
    var postDoc = AllGames(gameScores: fetchedGameList).toJson();
  ...
  makeCategoryInputField(String category, int playerIndex) {
    return Flexible(
      ...
      // },
      onSaved: (String value) {
        //var catScores = playerScores[playerIndex].categoryScores;
